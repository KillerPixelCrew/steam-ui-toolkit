...
leaves the page behind the panel, which on a controller is indistinguishable from a dead button.
The route itself goes through the shared `navigateSteamRoute` bound, so a host that answers with
