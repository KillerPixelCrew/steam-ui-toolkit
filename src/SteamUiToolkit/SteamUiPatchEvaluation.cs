...
public static class SteamUiPatchEvaluation
{
    ...
    {
        return EvaluateOutcomeAsync(context, role, expression, fallback, null, cancellationToken);
    }
    ...
    /// <param name="fallback">Diagnostic used when the page reported nothing usable.</param>
    /// <param name="inspect">
    ///     Reads the parsed answer and the page's own error, if any, before the
    ///     outcome is decided.
    /// </param>
    /// <param name="cancellationToken">Cancels the evaluation.</param>
    /// <returns>The patch operation result.</returns>
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var reported = root.TryGetProperty("error", out var error)
    ...
    ///     Boolean properties that must all be present and true. With none,
    ///     this is simply whether the page reported <c>ok</c>.
    /// </param>
    ...
    public static bool IsSuccessful(string value, params string[] requiredFlags)
    {
        ...
        try
        {
            ...
            {
                if (!root.TryGetProperty(flag, out var reported)
