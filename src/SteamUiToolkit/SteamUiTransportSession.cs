...
///     Whether a validated Steam target accepted the request at all. This is
...
///     changed shape" indistinguishable in a log.
/// </param>
...
public readonly record struct CefEvalResult(bool Reachable, string? Value, string? Error)
{
    ...
    public static CefEvalResult Ok(string? value)
    {
        return new CefEvalResult(true, value, null);
    }
    ...
    public static CefEvalResult Unreachable(string error)
    {
        return new CefEvalResult(false, null, error);
    }
}
...
public static class SteamUiTransportSession
{
    ...
    /// <remarks>
    ///     The host's master switch. Everything else checks this rather than assuming, so a
    ///     consumer can turn the whole surface off without tearing down the session.
    /// </remarks>
    ...
    ///     A different transport is already attached.
    ...
    ///     The transport being torn down. Detaching one that is not the
    ///     attached transport does nothing, so a late teardown cannot orphan a newer session.
    /// </param>
    ...
    /// <returns>
    ///     The outcome. Never throws: an unreachable target, a timeout and a cancellation all
    ///     come back as results carrying their reason, because a caller doing one-shot work against a
    ///     client that may not be running should not have to distinguish those with a catch block.
    /// </returns>
    ...
    /// <param name="cancellationToken">Cancels the evaluation.</param>
    /// <returns>
    ///     The outcome, on the same never-throws contract as
    ///     <see cref="EvaluateAsync(string, TimeSpan, CancellationToken)" />.
    /// </returns>
    public static Task<CefEvalResult> EvaluateOnVisibleWindowAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        return EvaluateAsync(SteamUiTargetRole.MainWindow, expression, timeout, cancellationToken);
    }
