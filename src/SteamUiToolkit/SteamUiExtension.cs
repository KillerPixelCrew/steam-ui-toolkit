...
public sealed class SteamUiExtensionManifest
{
    ...
    /// <remarks>
    ///     Also the prefix every patch the extension owns must use, so an extension cannot
    ///     name a patch that collides with the host's or another extension's.
    /// </remarks>
    [JsonPropertyName("id")]
    ...
    /// <remarks>
    ///     Checked for exact equality, not compatibility. The injected surface is coupled to a
    ///     Steam build and to this host's bridge; an extension built against a different contract has
    ///     no way to degrade gracefully, so refusing it with a clear reason is the honest outcome.
    /// </remarks>
    ...
    /// <remarks>
    ///     Declared rather than discovered so the host can refuse a collision before any
    ///     extension code runs, and so an extension that fails to load still has its patches removed.
    /// </remarks>
