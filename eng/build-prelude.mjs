...
// nothing. bridge.ts opens the IIFE, and ownership.ts and rpc.ts follow it. Every other top-level
// fragment is discovered and sorted, and so are the gates, so adding one is a new file and nothing
// else; components.ts is the row host and comes last so the asset reads as helpers, then gates, then
// the rows they render. This is the order WSGM's build-steam-assets.mjs composes, so every check run
// against the prelude sees the fragments where the shipped asset has them.
const fixed = ["types.ts", "bridge.ts", "ownership.ts", "rpc.ts"];
const discover = async (directory, exclude = []) =>
  (await readdir(directory, { withFileTypes: true }))
    .filter(
      (entry) => entry.isFile() && entry.name.endsWith(".ts") && !exclude.includes(entry.name),
    )
    .map((entry) => join(directory, entry.name))
    .sort();
const fragments = [
  ...fixed.map((name) => join(sourceDirectory, name)),
  ...(await discover(sourceDirectory, [...fixed, "components.ts", "epilogue.ts"])),
  ...(await discover(join(sourceDirectory, "gates"))),
